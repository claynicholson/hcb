...
class CardGrantsController < ApplicationController
  ...
  def create
    ...
    @card_grant = @event.card_grants.build(params.require(:card_grant).permit(:email, :amount_cents, :expiration_at, :purpose, :one_time_use, :pre_authorization_required, :invite_message, :instructions, :stripe_cards_enabled, :reimbursement_reports_enabled).merge(sent_by: current_user))
